...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1060'">
            <xsl:text>Način citiranja ieee-mod</xsl:text>
          </xsl:when>
        </xsl:choose>
      </xsl:when>
	...
	<xsl:template name="BibDisplayTitleJournal">
		...
		<xsl:if test ="$cTitle!=0">
			...
			<xsl:choose>
				<xsl:when test="$cJournalName!=0 or $cVolume!=0 or $cIssue!=0 or $cPages!=0 or $cYear!=0 or $cDOI!=0 ">
	...
	<xsl:template name="BibDisplayTitleAP">
		...
		<xsl:if test ="$cTitle!=0">
			...
			<xsl:choose>
				<xsl:when test="$cVolume!=0 or $cIssue!=0 or $cPages!=0 or $cYear!=0 or $cDOI!=0 ">
					<xsl:call-template name ="List_Separator_NoSpace"/>
					<xsl:call-template name="templ_prop_CloseQuote"/>
					<xsl:call-template name ="templ_prop_Space"/>
				</xsl:when>
	...
	<xsl:template name="BibDisplayAuthorReport">
		...
		<xsl:variable name="cThesisType">
			<xsl:value-of select="count(b:ThesisType)" />
		</xsl:variable>
		<xsl:variable name="cEditor">
			<xsl:value-of select="count(b:Author/b:Editor/b:NameList/b:Person)" />
		</xsl:variable>
			...
			<xsl:when test ="$cAuthors=0">
				<xsl:choose>
					<xsl:when test="$cCorporateAuthors!=0">
						<xsl:value-of select="b:Author/b:Author/b:Corporate"/>
						<xsl:choose>
							<xsl:when test="$cTitle!=0 or $cThesisType!=0 or $cCity!=0 or $cPublisher!=0 or $cYear!=0">
								<xsl:call-template name ="templ_prop_ListSeparator"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:call-template name ="templ_prop_Dot"/>
	...
	<xsl:template name="BibDisplayTitleReport">
		...
		<xsl:if test ="$cTitle!=0">
			...
			<xsl:choose>
				<xsl:when test=" $cThesisType!=0 or $cCity!=0 or $cYear!=0 or  $cPublisher!=0  ">
					<xsl:call-template name ="List_Separator_NoSpace"/>
					<xsl:call-template name="templ_prop_CloseQuote"/>
					<xsl:call-template name ="templ_prop_Space"/>
				</xsl:when>
...
<xsl:call-template name ="templ_prop_Space"/>
	...
	<xsl:template name="BibDisplayThesisType">
		<xsl:variable name="cThesisType">
			<xsl:value-of select="count(b:ThesisType)"/>
		</xsl:variable>
		<xsl:if test ="$cThesisType!=0">
				...
				<xsl:element name="tr">
					...
					<xsl:element name="td">
						...
						<xsl:element name="p">
							...
							<xsl:call-template name ="BibDisplayThesisType"/>
